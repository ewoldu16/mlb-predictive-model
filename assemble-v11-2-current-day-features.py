...
def lineup_frame(gid,lineup):
 rows=[]
 for side in ('away','home'):
  for player in lineup['teams'][side]:rows.append({'game_id':gid,'team_side':side,'batting_order':player['order'],'player_id':player['player_id'],'player_name':player['name'],'position':player.get('position'),'lineup_status':lineup['status'],'lineup_source':lineup['source'],'lineup_retrieved_at':lineup['retrieval_timestamp'],'source_player_id':player.get('source_player_id')})
 frame=pd.DataFrame(rows)
 if len(frame)!=18 or frame.duplicated(['team_side','batting_order']).any() or frame.duplicated(['team_side','player_id']).any():raise ValueError(f'{gid}: lineup did not normalize to 18 unique slots')
 return frame

def stage(day):
 folder=ROOT/'data/live'/day;games,_=fetch_schedule(day,folder/'schedule_cache');probable,probable_meta=fetch_probable_lineups(games,day,folder/'probable_lineup_cache');now=datetime.now(timezone.utc);eligible=[]
 ...
 for game in games:
  ...
  confirmed,_=fetch_lineup_status(game,folder/'boxscore_cache');gid=game['game_id'];details=confirmed_lineup_details(game,folder/'boxscore_cache') if confirmed=='confirmed' else (owner_lineup_for_game(ROOT,game) or probable.get(gid))
  if not details or not game.get('away_starter_id') or not game.get('home_starter_id'):continue
  lineup=lineup_frame(gid,details);cache=RAW/'lineups'/str(YEAR);cache.mkdir(parents=True,exist_ok=True);lineup.to_csv(cache/f'lineup_{gid}.csv',index=False);all_lineups=pd.concat([all_lineups[~all_lineups.game_id.eq(gid)],lineup],ignore_index=True)
  ...
  sr={'game_id':gid,'away_starter_id':game['away_starter_id'],'away_starter_name':game['away_starter'],'home_starter_id':game['home_starter_id'],'home_starter_name':game['home_starter']};starters=pd.concat([starters[~starters.game_id.eq(gid)],pd.DataFrame([sr])],ignore_index=True);eligible.append({'game_id':gid,'lineup_status':details['status'],'lineup_fingerprint':lineup_fingerprint(details)})
