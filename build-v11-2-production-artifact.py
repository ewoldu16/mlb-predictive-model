...
def main():
 ...
 artifact=OUT/'v11_2_compact_pipeline.joblib';joblib.dump(pipe,artifact);digest=hashlib.sha256(artifact.read_bytes()).hexdigest();runtime={name:version(name) for name in ['joblib','numpy','pandas','scikit-learn','scipy']};meta={'model_version':'V11.2_COMPACT_TEAM_RUN','artifact_format':'joblib','artifact_sha256':digest,'runtime_versions':runtime,'features':features,'feature_count':len(features),'poisson_alpha':10,'training_seasons':[2021,2022,2023,2024],'training_team_games':len(data),'imputation':'median fitted on 2021-2024','scaling':'standard fitted on 2021-2024','prediction_floor':.05,'negative_binomial':{'parameterization':'variance=mu+alpha*mu^2','dispersion_alpha_training_only':alpha},'frozen_specification':'results/v11_2_compact_frozen_specification.json'};(OUT/'v11_2_compact_metadata.json').write_text(json.dumps(meta,indent=2));print(json.dumps({'artifact':str(artifact),'sha256':digest,'nb_alpha':alpha,'features':len(features)},indent=2))
