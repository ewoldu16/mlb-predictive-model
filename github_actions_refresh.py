...
from mlb_app.storage import database_url,finish_rebuild_request,load_snapshots_for_date,load_state,pending_rebuild_requests,save_state
...
def _grade_previous_day(day):
 target=(datetime.fromisoformat(day).date()-timedelta(days=1)).isoformat();daily=ROOT/'results/live_tracking/v11_2_live_daily_summary.csv'
 if daily.exists():
  import pandas as pd
  if target in set(pd.read_csv(daily).date.astype(str)):return
 snapshots=load_snapshots_for_date(target)
 if not snapshots:return
 folder=Path(os.getenv('MLB_STATE_DIR',ROOT/'data/live'))/target;folder.mkdir(parents=True,exist_ok=True)
 for game in snapshots:atomic_json(game,folder/f"prediction_{int(game['game_id'])}.json")
 subprocess.run([sys.executable,str(ROOT/'evaluate-live-v11-2-day.py'),'--date',target],cwd=ROOT,check=True)
def run(day,force_season=False,skip_season=False):
 ...
 try:
  ...
  service=V112ModelService(ROOT);payload=refresh_cycle(ROOT,service,day);resolved=_resolve_requests(ROOT,payload);finished=datetime.now(timezone.utc).isoformat();state={'season_refresh_date':day if refresh_needed and not skip_season else _read_state().get('season_refresh_date'),'completed_today':completed,'last_run':finished};atomic_json(state,_state_path());_grade_previous_day(day);_publish_tracking();health={'status':'ok','executor':'github_actions','started_at':now,'last_successful_refresh':finished,'current_data_date':day,'games':len(payload.get('games',[])),'queued_requests_resolved':resolved,'full_season_refresh':bool(refresh_needed and not skip_season),'refresh_seconds':time.perf_counter()-started};save_state('refresh_status',health);print(json.dumps(health,indent=2));return payload
